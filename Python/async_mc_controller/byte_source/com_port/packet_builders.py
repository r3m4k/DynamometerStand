# -*- coding: utf-8 -*-
"""Создатели бинарных пакетов протокола.
...
class BasePacketBuilder(ABC):
    ...
    def _compute_crc(data: bytes) -> bytes:
        ...
        Метод может быть переопределён в наследнике.
...
class PacketBuilderHX711(BasePacketBuilder):
    """Абстрактный построитель пакетов протокола HX711.

    Задаёт заголовок HX711-протокола и обязывает наследников вернуть байт
    ...
    # Заголовок HX711-протокола (общий для всех наследников)
...
class PacketBuilderHX711Text(PacketBuilderHX711):
    """Создатель пакетов HX711 с текстовой командой.

    Упаковывает строковый текст команды в пакет протокола HX711 с форматом
...
class PacketBuilderHX711Bytes(PacketBuilderHX711):
    """Создатель пакетов HX711 с байтовой командой.

    Упаковывает готовую последовательность байтов в пакет протокола HX711
