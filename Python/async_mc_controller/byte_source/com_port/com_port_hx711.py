...
from async_mc_controller.byte_source.com_port.packet_builders import PacketBuilderHX711Text, PacketBuilderHX711Bytes
from async_mc_controller.signal_bus import bus
from async_mc_controller.logger import mc_logger

#########################

_logger: logging.Logger = mc_logger.get_logger('MC.ComPort.ComPortHX711')
...
class AsyncComPortHX711(AsyncComPort):
    """Асинхронный класс для работы с платой МК с АЦП HX711.
    ...
    Команды на МК собираются через билдеры протокола HX711
    (PacketBuilderHX711Text / PacketBuilderHX711Bytes) — заголовок и CRC
    ...
    # TODO: проверить правильность команд в прошивке МК
    _set_foo_stage_command:     bytes = PacketBuilderHX711Bytes.build_byte_command(bytes([0xAA, 0x01]))
    _set_measure_stage_command: bytes = PacketBuilderHX711Bytes.build_byte_command(bytes([0xAA, 0x02]))
    _init_handshake_command:    bytes = PacketBuilderHX711Text.build_text_command('HANDSHAKE_REQ')
    _heartbeat_command:         bytes = PacketBuilderHX711Text.build_text_command('HEARTBEAT_REQ')
    ...
    def __init__(self, port_name: str, baudrate: int,
        ...
        self._heartbeat_task: Optional[asyncio.Task] = None     # Таска для heartbeat loop
        self._stopped: bool = False     # Флаг остановки
    ...
    async def on_handshake_done(self) -> None:
        ...
        _logger.info(f'Рукопожатие с HX711 по порту {self._port_name} выполнено успешно')
    ...
    async def on_interrupt_measuring(self) -> None:
        ...
        Идемпотентен: повторный вызов после уже выполненной остановки
        (через STOP_MEASURING или INTERRUPT_MEASURING).
    ...
    async def send_text_command(self, text: str) -> None:
        ...
        Упаковывает текст через PacketBuilderHX711Text и отправляет с ожиданием
        ...
        """
        packet = PacketBuilderHX711Text.build_text_command(text)
