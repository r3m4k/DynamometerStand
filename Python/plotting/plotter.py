...
class Plotter:
    ...
    def update_canvas_config(self, new_canvas_config: CanvasConfig):
    ...
    def plotting_2d(self):
        """
        Построение графиков величин, имеющие две компоненты
        """
        if (self.canvas_config.n_rows != 2) and (self.canvas_config.n_cols != 1):
            raise RuntimeError('Неправильная сетка фигуры!\n'
                               'Должно быть: n_rows = 2, n_cols = 1\n'
                               f'Передано: n_rows = {self.canvas_config.n_rows}, n_cols = {self.canvas_config.n_cols}')
        self._canvas_plotting()
    ...
    def plotting_Ndim_static(self, dim: int):
        """
        Построение графиков статических величин, имеющие N компонент, а также гистограммы их распределений
        """
        if (self.canvas_config.n_rows != dim) and (self.canvas_config.n_cols != 2):
            raise RuntimeError('Неправильная сетка фигуры!\n'
                               f'Должно быть: n_rows = {dim}, n_cols = 2\n'
                               f'Передано: n_rows = {self.canvas_config.n_rows}, n_cols = {self.canvas_config.n_cols}')

        if isinstance(self.canvas_config.x_data, list) and isinstance(self.canvas_config.x_data[0], np.ndarray):
            for d in range(dim):
                self.canvas.plot(
                    [[self.canvas_config.x_data[d], np.full(self.canvas_config.x_data[d].shape, np.nan, dtype=float)] for _ in range(dim)],
                    [[self.canvas_config.y_data[d], np.full(self.canvas_config.y_data[i].shape, np.nan, dtype=float)] for i in range(dim)],
                    color_names=[[self.canvas_config.color_names[i], None] for i in range(dim)]
                )
        else:
            self.canvas.plot(
                [[self.canvas_config.x_data, np.full(self.canvas_config.x_data.shape, np.nan, dtype=float)] for _ in range(dim)],
                [[self.canvas_config.y_data[i], np.full(self.canvas_config.y_data[i].shape, np.nan, dtype=float)] for i in range(dim)],
                color_names=[[self.canvas_config.color_names[i], None] for i in range(dim)]
            )
        self.canvas.suptitle(self.canvas_config.suptitle, **self.canvas_config.suptitle_kwargs)
        ...
        if isinstance(self.canvas_config.x_label, str):
            self.canvas.set_axis_labels(x_label=[self.canvas_config.x_label, None],
                                        y_label=[[self.canvas_config.y_label[i], None] for i in range(dim)])
        elif isinstance(self.canvas_config.x_label, list):
            self.canvas.set_axis_labels(x_label=[[self.canvas_config.x_label[i], None] for i in range(dim)],
                                        y_label=[[self.canvas_config.y_label[i], None] for i in range(dim)])
        else:
        ...
        self.plotting_hline(
            dim=dim,
            values=[np.mean(self.canvas_config.y_data[n_row]) for n_row in range(dim)],
            colors=[self.canvas_config.dark_color_names[n_row] for n_row in range(dim)],
        ...
        for n_row in range(dim):
            ...
            ax.annotate(f'σ = {np.round(np.std(self.canvas_config.y_data[n_row]), 6)}', xy=(0.6, 0.88),
    ...
    def plotting_3d_static(self):
        ...
        self.plotting_hline(
            dim=3,
            values=[np.mean(self.canvas_config.y_data[n_row]) for n_row in range(3)],
    ...
    def plotting_hline(self, dim: int, values: list[float],  colors: list[str], annotations: list[str]=None):
        for n_row in range(dim):
            try:
                ax = cast(Axes, self.canvas.ax[n_row, 0])
            except IndexError:
